...
class Translator {
  ...
  translate(text, dict, titles, timeRegex, locale) {
    ...
    // Search for titles and add to matchesMap obj
    ...
    // Search for spaced word matches and add to matchesMap obj
    Object.entries(wordsWithSpace).map(([k, v]) => {
      if (lowerText.includes(k)) {
        matchesMap[k] = v;
      }
    });
    // Search for individual word matches and add to matchesMap obj
    lowerText.match(/(\w+([-'])(\w+)?['-]?(\w+))|\w+/g).forEach((word) => {
      if (dict[word]) matchesMap[word] = dict[word];
    });
    // Search for time matches and add to matchesMap obj
    ...
    // No matches
    if (Object.entries(matchesMap).length === 0) return null;
    // Return logic
